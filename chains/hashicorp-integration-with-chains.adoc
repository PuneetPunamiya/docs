...
=== In this doc, we will be running https://github.com/tektoncd/chains/blob/main/docs/tutorials/signed-provenance-tutorial.md[Chains Signed Provenance Tutorial] using `kms` solution by integrating Tekton Chains with Hashicorp Vault

*NOTE* * Prerequisite * Hashicorp vault should be installed * This
provider also requires that the `transit` secret engine is enabled * If
not done, you can login into the vault provider and run the following
command
...
=== https://github.com/tektoncd/chains/blob/main/docs/tutorials/signed-provenance-tutorial.md[Chains Signed Provenance Tutorial] using `kms` solution by integrating with Hashicorp Vault

* Step 1: Generate a Key Pair

This provider requires that the standard Vault environment variables
(`$VAULT_ADDR, $VAULT_TOKEN`) are set correctly.
...
NOTE:- If you enabled transit secret engine at different path with the
use of -path flag (i.e., $ vault secrets enable -path=``someotherpath''
transit), you can use TRANSIT_SECRET_ENGINE_PATH environment variable to
specify this path while generating a key pair like the following:
...
[arabic]
. The Chains controller will be pushing signatures to an OCI registry
using the credentials linked to your TaskRun’s service account. See our
authentication
https://github.com/tektoncd/chains/blob/main/docs/authentication.md[doc]
. The Kaniko Task that will build and push the image needs push
permissions for your registry.

To set up auth for the Kaniko Task, you’ll need a Kubernetes secret of a
docker config.json file which contains the required auth. You can create
the secret by running:
...
You can add the above changes to the TektonConfig CR and correspondingly
`chains-config` configMap will be updated by the operator
...
Substitute with the URL of the registry where you want to push the
...
* Wait for a minute to allow Tekton Chains to generate the provenance
and sign it, and then check the availability of the
chains.tekton.dev/signed=true annotation on the task run.
